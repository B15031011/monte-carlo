...
# 逐年计算分解
# raw_data <- read.csv("03_table2_quantitative indicators.csv")
...
# )
# write.csv(result, "contribution_rate_result2.csv")
...
for (index in (1:4)) {
  ...
  co2_gdp_results[index] <- Dz_vect[4]*co2_data[5*index-4]
  co2_energy_results[index] <- Dz_vect[5]*co2_data[5*index-4]
  co2_population_results[index] <- Dz_vect[6]*co2_data[5*index-4]
  gdp_population_results[index] <- Dz_vect[7]*co2_data[5*index-4]
  energy_gdp_results[index] <- Dz_vect[8]*co2_data[5*index-4]
...
write.csv(result, "population_gap_amount_result_align.csv")
